...
mat init_board_gaus(int N, int k){
	...
	for (int i=0; i<k; i++){
		double ux = -1 + static_cast<double> (std::rand()/(static_cast<double>(RAND_MAX/(1+1)))); // generate mean between -1 and 1
		double uy = -1 + static_cast<double> (std::rand()/(static_cast<double>(RAND_MAX/(1+1))));
		double sig = 0.05 + static_cast<double> (std::rand()/(static_cast<double>(RAND_MAX/(0.5-0.05))));

		std::normal_distribution<double> norm_x(ux, sig);
		std::normal_distribution<double> norm_y(uy, sig);

		int counter = 0;
		while(counter<n){
			double dat_x = norm_x(generator);
			double dat_y = norm_y(generator);
			if (dat_x<1 && dat_y<1){
...
int main(void){
	...
	mat data = init_board_gaus(200,3);					// Generate data.
